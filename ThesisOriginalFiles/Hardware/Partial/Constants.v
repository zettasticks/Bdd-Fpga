...
`define HASH_TABLE_HASH(e,t,var) (e + t + var) % `HASH_SIZE
...
`define CONTEXT_SIZE (2**`CONTEXT_PTR_SIZE)
...
`define NEGATE_END 		(`TYPE_UPPER_END + 1)
`define QUANTIFY_END    (`NEGATE_END + 1)
`define TOP_LOWER_END   (`QUANTIFY_END + 1)
`define TOP_END  		(`TOP_LOWER_END + `VAR_SIZE - 1)
`define LEFT_INDEX_LOWER_END (`TOP_END + 1)
`define LEFT_INDEX_UPPER_END (`LEFT_INDEX_LOWER_END + 3 - 1)
`define RIGHT_INDEX_LOWER_END (`LEFT_INDEX_UPPER_END + 1)
`define RIGHT_INDEX_UPPER_END (`RIGHT_INDEX_LOWER_END + 3 - 1)
`define STORE_ONLY_END (`RIGHT_INDEX_UPPER_END + 1)
`define REQUEST_END 	`STORE_ONLY_END

// Extract from node
`define NODE_NEXT(node)  node[30:0]
`define NODE_ELSE(node)  node[62:32]
`define NODE_THEN(node) node[94:64]
...
`define REQUEST_LEFT_INDEX(req) req[`LEFT_INDEX_UPPER_END:`LEFT_INDEX_LOWER_END]
`define REQUEST_RIGHT_INDEX(req) req[`RIGHT_INDEX_UPPER_END:`RIGHT_INDEX_LOWER_END]
`define REQUEST_STATE(req) req[`STATE_UPPER_END:`STATE_LOWER_END]
`define REQUEST_NEGATE(req) req[`NEGATE_END]
`define REQUEST_TOP(req) req[`TOP_END:`TOP_LOWER_END]
`define REQUEST_QUANTIFY(req) req[`QUANTIFY_END]
`define REQUEST_STORE_ONLY(req) req[`STORE_ONLY_END]
